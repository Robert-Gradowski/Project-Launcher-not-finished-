...
namespace DiamondStories
{
    class CheckFiles
    {
        string[] steamlegalfiles = new string[]
        {
            "\\bass.dll",
            "\\vorbisHooked.dll",
            "\\cleo\\blur.cs",
            ...
            "\\models\\txd\\dscontrols.txd",
            "\\models\\txd\\NTD_RESOURCES.txd",
        ...
        public void DeleteFilesExcept(string directory, Label la)
        {
            var files = Directory.GetFiles(directory).Where(x => !legalfiles.Contains(Path.GetFileName(x)));
            foreach (var file in files)
            {
                File.Delete(file);
            }
            Session ses = new Session();
            ses.SendSession(Client.id, la);
        }
    }
}
